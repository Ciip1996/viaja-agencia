...
const FEATURE_META: Record<string, { label: string; icon: typeof Car }> = {
  AC: { label: "Aire acondicionado", icon: Snowflake },
  GPS: { label: "GPS", icon: Navigation },
  automatico: { label: "Automatico", icon: Cog },
  manual: { label: "Manual", icon: Cog },
  "4-pasajeros": { label: "4 pasajeros", icon: Users },
  "5-pasajeros": { label: "5 pasajeros", icon: Users },
  "7-pasajeros": { label: "7 pasajeros", icon: Users },
  seguro: { label: "Seguro incluido", icon: ShieldCheck },
  gasolina: { label: "Gasolina", icon: Fuel },
  diesel: { label: "Diesel", icon: Fuel },
  bluetooth: { label: "Bluetooth", icon: Gauge },
};
    ...
    <>
      ...
      <section className="relative flex min-h-[50vh] items-center justify-center overflow-hidden bg-primary">
        <div className="absolute inset-0 z-0">
          <Image
            src="https://images.unsplash.com/photo-1449965408869-ebd3fee75e46?w=1920&q=80"
            alt="Auto de lujo en carretera"
            fill
        ...
        >
          <p className="mb-3 font-body text-sm font-semibold uppercase tracking-[0.25em] text-white/70">
            Movilidad sin Limites
          </p>
          <h1 className="font-heading text-5xl font-bold text-white sm:text-6xl md:text-7xl">
            Renta de{" "}
            <span className="text-gradient-gold inline-block">Autos</span>
          </h1>
          <p className="mx-auto mt-4 max-w-2xl font-body text-base text-white/80 sm:text-lg">
            Encuentra el vehiculo perfecto para tu viaje. Desde economicos hasta
            autos de lujo, con las mejores tarifas y cobertura total.
          </p>
        </motion.div>
      </section>
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Lugar de devolucion"
            ...
            <div className="relative">
              ...
              <input
                ...
                className="h-12 w-full rounded-xl border border-border bg-surface pl-10 pr-4 font-body text-sm text-text outline-none transition-colors focus:border-secondary focus:ring-1 focus:ring-secondary"
                aria-label="Fecha de devolucion"
              />
            </div>
            <div className="relative">
              <Clock className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted" />
          ...
          <AnimatedSection variant="fade-up" className="mb-8">
            <div className="flex flex-wrap items-center gap-3">
              <p className="mr-2 font-body text-sm text-text-muted">
                {filteredCars.length} vehiculos disponibles
              </p>
              <button
                type="button"
              ...
              {(
                Object.entries(CATEGORY_LABELS) as [CarCategory, string][]
              ).map(([key, label]) => (
                <button
                  key={key}
                  type="button"
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    <div className="mb-4 flex items-center gap-4 text-text-muted">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="font-body text-xs">{car.seats} asientos</span>
                      </div>
                      <div className="flex items-center gap-1">
