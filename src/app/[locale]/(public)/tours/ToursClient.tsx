...
const CATEGORY_ICONS: Record<TourCategory, typeof Compass> = {
  cultura: Landmark,
  aventura: Compass,
...
export default function ToursClient({ cms = {} }: { cms?: Record<string, string> }) {
  const t = useTranslations("pageTours");
  const tc = useTranslations("common");

  const heroImage = cms.tours_hero_image || "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=1920&q=80";
  const heroTag = cms.tours_hero_tag || t("heroTag");
  const heroHeading = cms.tours_hero_heading || t("heroHeading");

  const categoryLabels: Record<TourCategory, string> = {
    cultura: t("categoryCulture"),
    aventura: t("categoryAdventure"),
    gastronomia: t("categoryGastronomy"),
    naturaleza: t("categoryNature"),
  };

  const difficultyLabels: Record<string, string> = {
    facil: t("difficultyEasy"),
    moderado: t("difficultyModerate"),
    dificil: t("difficultyHard"),
  };
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_auto]">
            ...
            <div className="relative">
              ...
              >
                ...
                {(
                  Object.entries(categoryLabels) as [TourCategory, string][]
                ).map(([key, label]) => (
